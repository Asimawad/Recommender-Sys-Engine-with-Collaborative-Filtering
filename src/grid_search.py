...
experiment_folder = setup_experiment_folder('ml-100k',experiment_name)
logger = setup_logging(experiment_folder)

# Define the hyperparameter grid to search lambda_reg:0.1 , gamma:0.01  taw: 0.1, K_factors: 30
param_grid = {
    'lambda_reg': [0.01, 0.1, 0.5, 1, 2],  # Regularization parameter
    'gamma': [0.01,0.05, 0.1,0.5,1],           # Learning rate for users' factors
    'taw': [0.05, 0.1,0.5, 1, 10, 100],                 # Weighting factor for genre features
    'K_factors':    [2,4,8,16,32]        # Number of latent factors [8,16, 25,32,40, 50] 
}

# source_data_folder = f"Training_data/{Dataset}"
source_data_folder = "Training_data/ml-100k"
